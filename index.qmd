...
Comencemos con la descarga de datos [acá](https://1drv.ms/f/c/3ba457a6b989f5e0/Ej_lJp_rhaJHkjIZgoO6OLcBkeMpK0uyc1uBp45jzPvOVA?e=acDGPL)
...
Analicemos la riqueza léxica en usuarios de reddit

Los datos están [acá](https://1drv.ms/f/c/3ba457a6b989f5e0/Ej_lJp_rhaJHkjIZgoO6OLcBkeMpK0uyc1uBp45jzPvOVA?e=6QMI6W)
...
comments <- read_csv("data/comments_reddit.csv")
posts <- read_csv("data/posts_reddit.csv")
...
comments_subreddit %>% 
  group_by(subreddit) %>% 
  summarise(frecuencia = n()) %>% 
  ggplot(aes(x = subreddit, y = frecuencia)) +
...
Con los mismos datos de reddit, genera el gráfico de palabras claves para los subreddit **argentina** y **chile**

Esta vez vamos a remover **stopwords** y **signos de puntuación**

## Comentario emojis

**Aun tenemos caracteres extraños**

😀 😍 🤖 😂 🙌

. . .

Podemos sacarlos con esta expresión regular [\\p{So}\\p{Cn}]


p{So}: Symbol or other

p{Cn}: Codepoints Not Assigned
...
# Queremos las primeras 5 componentes
# n_neighbors determina la construcción de un grafo
...
# clusters contiene el cluster asignado a cada uno de los 500 documentos 
X_topics = []
for c in topic_ids:
    idx = np.where(clusters == c)[0] # indices del cluster c. Contiene los documentos de cada cluster
...
# ravel aplana los elementos, para que no tengan dos dimensiones, sino solo una. 
# Eso permite apilar filas
...
- Encontrar palabras clave para cada tópico
- Encontrar documentos representativos de cada tópico
...
Los tópicos no tienen nombres interpretables
...
Cargaremos archivos de un modelo ya entrenado con datos de ~200.000 noticias
...
# Cargar títulos de noticias ~200.000
data = pd.read_parquet("data/titulos.parquet")
docs = list(data.text)
del data
...
# Cargar embeddings en d2. Contiene la conversión de los datos a vectores
...
## ¿Cómo mejorar los nombres?

Estos son los tópicos que salieron de un procesamiento anterior

```{python, echo = TRUE}
info =  topic_model.get_topic_info()
info[["Name", "Representation"]][1:4]
```

## Trabajando con Gemini

Usaremos un LLM, para que "bautice" nuestros tópicos
...
generation_config = genai.types.GenerationConfig(
    max_output_tokens=100, # para que 
    temperature=0.7, # regula la creatividad 
    candidate_count=1 # queremos solo una respuesta
)

# Le mandamos el prompt al modelo
prompt = "Hazme un resumen del libro El Principito"
response = model.generate_content(prompt, generation_config=generation_config)
print(response.text)
```

## Pidiendo títulos para tópicos

Bertopic entrega dos insumos interesantes
...
Veamos cómo implementar para un tópico 

. . .

```{python, echo = TRUE}

# Tomamos el primer tópico del modelo entrenado previamente
info =  topic_model.get_topic_info()
first_topic = info[1:2]

# Bertopic encuentra palabras representativas
palabras_topico = list(first_topic["Representation"]) 
palabras_principales = [palabra for palabra in palabras_topico[:5]][0]
palabras_texto = ", ".join(palabras_principales)

# COmbinamos los textos representativos del tópico en un solo gran texto
docs_representativos = list(first_topic['Representative_Docs'])[0]
contexto_docs = ". ".join(docs_representativos)

# Creamos un prompt. 
# Se insertan en el prompt la información que devuelve bertopic 
...
## Carguemos los títulos creados con Gemini

La idea es hacer el proceso anterior iterativamente, para cada uno de los tópicos

. . .

Este proceso es lento zzz. Yo ya lo hice
...
Experimentaremos con un modelo de IA

. . .

Utiliza los datos de reddit, para identificar lenguaje ofensivo. La idea es construir un prompt que logre capturar ese tipo de interacciones 

. . .

Trabajaremos en python, usando google colab

**Pasos**:

1) Explorar google colab (a través de gmail)

2) Obtener API Key para modelo de gemini (entrar a aistudio.google.com)

3) Escribir el código para cumplir la misión   
